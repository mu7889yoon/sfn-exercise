...
exports.handler = async (event) => {
  ...
  const countParam = Number(event.queryStringParameters?.count);
  const shuffleParam = event.queryStringParameters?.shuffle;
  const shuffle = shuffleParam !== 'false' && shuffleParam !== '0';
  const quizId = new Date().toISOString();

  let quiz;
  try {
    quiz = await buildQuizFromSeed(quizId, countParam, shuffle);
  } catch (err) {
